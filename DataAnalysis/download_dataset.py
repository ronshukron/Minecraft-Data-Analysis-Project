...
def relpaths_to_download(relpaths, output_dir):
    ...
    # Convert non_defect to a set for faster lookups
    non_defect_set = set(non_defect)

    # Use a list comprehension to keep items in relpaths that are not in non_defect_set
    diff_to_download = [path for path in relpaths if path not in non_defect_set]

    print(f'total: {len(relpaths)} | exist: {len(non_defect)} | downloading: {len(diff_to_download)}')
